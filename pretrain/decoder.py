...
class Model_decoder(nn.Module):
    ...
    def topo_pred(self, mol_batch, node_rep, super_node_rep, graph_batch):
        bond_if_loss, bond_if_auc = 0, 0
        ...
        super_rep = super_node_rep.to(self.device)
        ...
        for mol_index, mol in enumerate(mol_batch): 
            ...
            if num_bonds<1: 
                ...
                mol_rep = node_rep[graph_batch == mol_index].to(self.device) 
                ...
                bond_if_target = a.index_put(indices=[mol.edge_index[0,:], mol.edge_index[1,:]], values=torch.tensor(1.)).to(self.device) 
                ...
                start_rep = mol_atom_rep_proj.index_select(0, mol.edge_index[0,:].to(self.device)) 
                end_rep = mol_atom_rep_proj.index_select(0, mol.edge_index[1,:].to(self.device))
            
                bond_type_input = torch.cat([start_rep, end_rep], dim=1) 
                bond_type_pred = self.bond_type_s(bond_type_input)  

                bond_type_target = mol.edge_attr[:,0].to(self.device)  
                ...
                atom_type_pred = self.atom_type_s(mol_rep) # [num_atom, MAX_ATOM_TYPE]

                atom_type_target = mol.x[:,0].to(self.device)  
        ...
        results = [bond_if_auc/mol_num, bond_type_acc/mol_num, atom_type_acc/mol_num, atom_num_rmse, bond_num_rmse] 

        return loss_tur, results

    def forward(self, mol_batch, node_rep, super_node_rep, graph_batch):

        loss_tur, results = self.topo_pred(mol_batch, node_rep, super_node_rep, graph_batch)
        loss = 0
