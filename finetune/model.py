...
class CentralityEncoding(nn.Module):
    ...
    def forward(self, dgl_graph):
        ...
        centrality_enc = self.linear(deg) 
...
class FineTunedTransformerModel(nn.Module):
    def __init__(self, pretrained_model, hidden_dim, num_classes):
        ...
        self.out_proj = nn.Linear(hidden_dim, int(hidden_dim / 2))
        self.node_classifier = nn.Linear(int(hidden_dim / 2), num_classes)

    def forward(self, batched_data):
        node_embeddings, _ = self.pretrained_model(batched_data)

        output = torch.relu(self.out_proj(node_embeddings))
        node_logits = self.node_classifier(output)  

        node_probs = torch.sigmoid(node_logits[:, 1])  

        graph_prob = global_max_pool(node_probs, batched_data.batch.to(node_probs.device))  

        graph_logits = torch.log(graph_prob / (1 - graph_prob + 1e-8))
